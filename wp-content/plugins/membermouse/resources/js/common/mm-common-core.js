...
function applyTableCheckboxes(table){var mainBox=jQuery('TH.checkbox INPUT',table);mainBox.unbind('click');mainBox.bind('click',function(){if(this.checked){jQuery('TD.checkbox INPUT',table).attr('checked','checked');}
...
else{this.validated=true;}},});var MM_iFrame=Class.extend({init:function(url,iframeId){this.iframeUrl=url;this.iframeId=iframeId;this.formId="mm-user-defined-form";this.appendToId="body";this.formObj=null;this.setupForm();},setupForm:function(){this.formObj=jQuery('<form action="'+this.iframeUrl+'" target="'
...
else{jQuery(textarea).val(jQuery(textarea).val()+text);}}});var smartTagLibDialog=new MM_DialogJS();var stl_js=new MM_SmartTagLibraryViewJS("MM_SmartTagLibraryView","");var MM_PaymentUtilsViewJS=MM_Core.extend({showPaymentOptions:function(userId,accessType,accessTypeId,lastActionParams){var values={mm_module:"payment_options",userId:userId,accessType:accessType,accessTypeId:accessTypeId,lastActionParams:lastActionParams};mm_pymtdialog.showDialog("mm-payment-options-dialog",this.module,465,570,"Payment Options",values,"","mm_pymtdialog");},getPaymentOptionsList:function(){productId=jQuery("#mm-product-selector").val();userId=jQuery("#mm-user-id").val();if(productId!=0&&userId!=0){var values={mm_action:"getPaymentOptionsList",mm_product_id:productId,mm_user_id:userId};var ajax=new MM_Ajax(false,this.module,this.action,this.method);ajax.send(values,false,"pymtutils_js","paymentOptionsListCallback");}
